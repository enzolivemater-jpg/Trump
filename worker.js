  ...
  async fetch(request) {
    ...
    try {
      upstream = await fetch(target.toString(), {
        ...
        // Reddit et Google News limitent au débit les IP de datacenter, et un
        // Worker partage ses IP avec tous les autres clients Cloudflare : d'où
        // les HTTP 429. Un cache de trois minutes divise par plusieurs les
        // requêtes réellement émises vers l'amont et fait passer ces refus,
        // tout en restant très en deçà du rythme de rafraîchissement du desk.
    ...
    // Un 429 de l'amont n'est pas définitif : on retente une fois après une
    // courte pause, ce que le navigateur ne peut pas faire aussi vite.
    if (upstream.status === 429 || upstream.status === 503) {
      ...
      } catch (e) { /* on garde la réponse d'origine */ }
    }
